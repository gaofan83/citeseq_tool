...
UMI_LENGTH=9
TAG="ADT.txt"
OUTPUT="feature_count.txt"
THREAD=8

function usage()
{
    echo "./feature_seq_count.sh [options]"
    echo "-h --help"
    ...
    echo ""
    echo "Example below:"
    echo "bash feature_seq_count.sh -fqc=SRR5808750_1.fastq.gz -fqf=SRR5808750_2.fastq.gz -it=ADT.txt -bcs=1 -bcl=16 -umis=17 -umil=9 -t=8 -o=SRR5808750_count.txt"
}
...
run_adt(){
	...
	awk -v mypattern="$pattern" 'BEGIN { for (i=2; i<= length(mypattern); i++) print substr(mypattern,1,i-1)"."substr(mypattern,i+1)}' > temp/temp.$pattern
